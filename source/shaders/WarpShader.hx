...
class WarpShader extends FlxShader // modified from https://www.shadertoy.com/view/wlScRz
{
	...
	float transformStrength = vec2(0.4);

	vec4 perm(vec4 x)
	{
		x = ((x * 34.0) + 1.0) * vec4(x);
		return x - floor(x * (1.0 / vec4(289.0))) * vec2(289.0);
	}
	...
	float noise2d(vec2 p)
	{
		...
		vec4 o1 = fract(k3 * vec2(0.0244));
		vec4 o2 = fract(k4 * vec2(0.0244));
